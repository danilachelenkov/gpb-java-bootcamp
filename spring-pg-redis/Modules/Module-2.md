...
Тут думаю,что до следующего шага нам надо создать простейшую миграцию, чтобы запусить проект и посмотреть консоль, что все конектиться, а так как там зависимость, он нам не даст без Ликви это сделать, поэтому мы создаем файл db/changelog/db.changelog-master.yaml и в нем описываем какой файл запустить , то есть второй файл, второй файл назовем db/changelog/changes/v1/create-table-test.yaml и в нем напишите что хотите минимально - два атрибута таких как ключ и текстовое поле
</span>
